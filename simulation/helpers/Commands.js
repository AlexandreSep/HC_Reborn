...
var g_Commands = {
    ...
    "ChooseHero": function (player, cmd, data) {
        var cmpTechnologyManager = QueryPlayerIDInterface(player, IID_TechnologyManager);
        let civTechs = cmpTechnologyManager.GetCivData(cmd.civ); // get the civchoice tech paths
    ...
    "CampaignYesNo": function (player, cmd, data) {
    },

    //Stops the cinematic camera
    "StopCinematicCamera": function (player, cmd, data) {
    ...
    "AIDialog": function (player, cmd, data) {
        let DialogData =
        {
            "type": "AIDialog",
            "players": [0],
            "targetPlayers": cmd.players, // the players to show this message to (from AI to enemies or from AI to allies for example)
            "character": cmd.character, // the name of the character to show on screen like: Impa, Agitha, Mido, BEWARE! CASE SPECIFIC!
            "dialogue": cmd.dialogue, // the actual dialogue text, make use of '||' brackets to give RGB colors like so: ||some words||255 0 0|| to make 'some words' red.
            "soundIndex": cmd.soundIndex, // represents the index of the played character sound ( -1 = no sound, 0 = random sound of the total, 1-x = specific sound with that index)
            "portraitSuffix": cmd.portraitSuffix, // _annoyed, _angry, _defeated, _neutral, _sad, _victorious, _happy
            "sender": player,
            "metaData": cmd.metaData
        };

        if (cmd.delay > 0) // if a delay was specified, call a timer instead
        {
            let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
            cmpTimer.SetTimeout(SYSTEM_ENTITY, IID_GuiInterface, "PushNotification", cmd.delay, DialogData);
            return;
        }
        //otherwise, call the message right now
        let cmpGUIInterface = Engine.QueryInterface(SYSTEM_ENTITY, IID_GuiInterface);
        cmpGUIInterface.PushNotification(DialogData);
    },

    "AddResource": function (player, cmd, data) {
        var cmpPlayer = QueryPlayerIDInterface(player, IID_Player);

        if (cmpPlayer)
            cmpPlayer.AddResource(cmd.resourceType, cmd.amount);
    },
    //HC-end
};
...
//HC-code, Battalion specific functionality for command operations
/**
 * Filter a selection of entities down to each individual battalion
 * @param {Number[]} entities -
 * @return {Number[]} -
 */
function GetBattalionIndices(entities)
{
    ...
    for (let ent of entities) {
        ...
        for (let index of BattalionIndices) {
            if (index == cmpBattalion.ownBattalionID) {
                addIndex = false;
                break;
            }
        }
        ...
        if (addIndex == true)
            BattalionIndices.push(cmpBattalion.ownBattalionID);
    }

    return BattalionIndices;
}

/**
 * @param {Vector2D} destination -
 * @param {Vector2D} startLocation -
 * @param {Map<Number, Object>} allBattalions -
 * @param {Number[]} battalionIndices -
 * @returns {Vector2D[]} a list of offset destinations based upon an initial destination for Battalions
 */
function GetBattalionDestinations(destination, startLocation, battalionIndices, allBattalions) {
    let destinations = [];
    let angle = startLocation.angleTo(destination); // the angle for the offset to rotate towards

    let currentOffsetModifierPositive = 0; // keeps track of the current horizontal positive offset modifier
    let currentOffsetModifierNegative = 0; // keeps track of the current horizontal negative offset modifier
    let depthModifier = 0; // keeps track of the current depth offset modifier
    let maxDepth = 5; // keeps track of the maxdepth for the current set of horizontal battalions, should be at least 5 by default

    for (let i = 0; i < battalionIndices.length; i++) {
        let battalionIndex = battalionIndices[i];
        let formationController = Engine.QueryInterface(allBattalions.get(battalionIndex)[0], IID_UnitAI).formationController;
        if (formationController == INVALID_ENTITY) { // if this battalion doesnt have a formation controller, push the current destination for this index and skip
            destinations.push(destination);
            continue;
        }

        let formationSize = Engine.QueryInterface(formationController, IID_Formation).GetSize();
        let modifiedWidth = formationSize.width * 0.75; // use half the width + 25% leeway
        if (modifiedWidth < 5)
            modifiedWidth = 5; // offsets should be at least of some significance to make them operatable

        let offset = new Vector2D(0, 0);
        if (i % 5 == 0) // add a depth offset every 5th battalion, and reset the current horizontal offsets for the next column
        {
            depthModifier -= maxDepth; // use whatever the maxdepth was for the current column and subtract that to the current depthmodifier
            maxDepth = 5; // depth should be at least 5 by default
            // initialize both negative and positive offsets to half the formation width of the centre battalion
            // combine half the width of the centre battalion with half the width of the next iterations
            currentOffsetModifierPositive = modifiedWidth;
            currentOffsetModifierNegative = -modifiedWidth;
            offset = new Vector2D(0, depthModifier); // this is the first battalion for this column, so no horizontal modifiers required yet
        }
        else if (i % 2 == 0) // every other iteration, set negative and positive horizontal offsets for battalions
        {
            currentOffsetModifierNegative -= modifiedWidth;
            offset = new Vector2D(currentOffsetModifierNegative, depthModifier);
            currentOffsetModifierNegative -= modifiedWidth;
        }
        else {
            currentOffsetModifierPositive += modifiedWidth; // combine half the width from the previous formation with the current one
            offset = new Vector2D(currentOffsetModifierPositive, depthModifier);
            currentOffsetModifierPositive += modifiedWidth; // add half the width and combine with the next iteration
        }

        let depth = formationSize.depth * 2.5;
        if (depth > maxDepth) //update the highest maxdepth from this column of battalions
            maxDepth = depth;

        offset.rotate(angle); // rotate the offset using the direction angle
        destinations.push(offset.add(destination));
    }
