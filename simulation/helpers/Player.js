...
function LoadPlayerSettings(settings, newPlayers)
{
	...
	for (let i = 0; i < numPlayers; ++i)
	{
		...
		if(settings.TriggerScripts != undefined) {
		...
		if (settings.AllowHeroSelectionDuringMatch){
			cmpPlayer.heroSettings.allowHeroSelectionDuringMatch = settings.AllowHeroSelectionDuringMatch;
		}
		if (settings.HeroCount){
			cmpPlayer.heroSettings.heroCount = settings.HeroCount;
		}
		//HC-End
