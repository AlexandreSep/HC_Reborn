var CONQUESTAI = function (m) {
     ...
     */
    m.HQ.prototype.loadConfigData = function (gameState)
    {
        let civref = gameState.playerData.civ;
        this.configData = {};
        for (let element in this.Config.AIData)
        {
            this.configData[element] = [];
            if (!this.Config.AIData[element][civref])
                continue;

            for (let data of this.Config.AIData[element][civref])
            {
                this.configData[element].push(data);
            }
        }
    };

    /**
     * load AI Dialog Data for this civ hero to make use of later
     */
    m.HQ.prototype.loadAIDialogData = function (gameState)
    {
        let civref = gameState.playerData.civ;
        let playerName = gameState.getPlayerName().split(" ")[0];
        this.AIDialogData = {};

        for (let civ in this.Config.AIDialogData)
        {
            if (civ != civref) // only go through the chosen AI civ
                continue;

            for (let heroName in this.Config.AIDialogData[civ])
            {
                if (heroName.split(" ")[0] != playerName)
                    continue;

                for (let dialogType in this.Config.AIDialogData[civ][heroName]) {
                    this.AIDialogData[dialogType] = [];
                    for (let data of this.Config.AIDialogData[civ][heroName][dialogType])
                        this.AIDialogData[dialogType].push(data);
                }
                break; // hero name found, no need to continue this operation
            }
        }
    };
