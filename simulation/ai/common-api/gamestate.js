var API3 = function (m) {
    ...
    m.GameState.prototype.init = function (SharedScript, state, player) {
        ...
        this.phases = [];
        let cctemplate = this.getTemplate(this.applyCiv("structures/{civ}/{civ}_civil_centre"));
        if (!cctemplate)
            return;
    ...
    {
        let dialogList = AIDialogData[dialogueType];
        if (dialogList == undefined)
            return;

        let randDialog = dialogList[randIntExclusive(+0, +dialogList.length)]; //get a random element from the dialogtype list
        Engine.PostCommand(PlayerID,
        {
            "type": "AIDialog",
            "players": players,
            "character": AIDialogData["PortraitName"][0], //use the portraitname specified in the config file
            "dialogue": randDialog.dialog,
            "soundIndex": randDialog.soundIndex,
            "portraitSuffix": randDialog.portraitSuffix,
            "delay": delay,
            "metaData": metaData
        });
    };
    ...
    m.GameState.prototype.getPlayerName = function () {
    ...
    /** This returns only units from buildings. */
    m.GameState.prototype.findTrainableUnitsOnly = function (classes, anticlasses) {
        let allTrainable = [];
        let civ = this.playerData.civ;
        this.getOwnStructures().forEach(function (ent) {
            let trainable = ent.trainableEntities(civ);
    ...
    m.GameState.prototype.getTraderTemplatesGains = function () {
        ...
        let supportTraderTemplateName = this.applyCiv("units/{civ}_trader_b");
