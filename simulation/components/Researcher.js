...
Researcher.prototype.GetTechnologiesList = function()
{
	...
	// HC-Code: Disable technologies of a group, if the max number is reached
	// If researched + inprogress reaches the max, disable the group temporarily, because the item could be removed from the queue by a player,
	// Only when researched reaches max, do we set this group as finished
	for (let groupName in cmpTechnologyManager.technologyGroups) {
		if(cmpTechnologyManager.technologyGroupsMap.get(groupName)) continue; // check if disabled already

		let technologyGroup = cmpTechnologyManager.technologyGroups[groupName];
		let researched = 0;
		let inProgress = 0;
		for(let member of technologyGroup.members) {
			if(cmpPlayer.disabledTechnologies[member]) cmpPlayer.disabledTechnologies[member] = false;
			if(cmpTechnologyManager.IsTechnologyResearched(member)) researched++;
			else if (cmpTechnologyManager.IsInProgress(member)) inProgress++;
		}

		if ((researched + inProgress) >= technologyGroup.maxAllowed) {
			for (let member of technologyGroup.members) {
				if (!this.IsTechnologyResearchedOrInProgress(member)) 
					cmpPlayer.AddDisabledTechnology(member);
			}
			if(researched >= technologyGroup.maxAllowed) cmpTechnologyManager.technologyGroupsMap.set(groupName, true); // set finished
		}	
	}
	// HC-End
	...
	for (const tech of techs)
	{
		...
		// HC-Code: Hide technology, if its requirements are not researched
		if (!this.requiredTechnologiesAreResearched(template)){
			continue;
		}
		// HC-End
