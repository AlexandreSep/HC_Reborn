...
Foundation.prototype.InitialiseConstruction = function(template)
{
	...
	cmpPlayer.AddPopulation (this.batalionSlotsCost);
...
Foundation.prototype.OnOwnershipChanged = function(msg)
{
	...
	// HC-Code
	cmpPlayer.AddPopulation(-1 * this.batalionSlotsCost);
};
...
Foundation.prototype.Build = function(builderEnt, work)
{
	...
	if (this.maxProgress >= 1.0)
	{
		...
		// HC-Code
		// We need this to spawn plots when builders construct a building that has plots attached to it
		...
		// HC-Code
		// HC-TODO: Needs to be checked.
		let cmpPlayer = QueryOwnerInterface(building);
		cmpPlayer.AddBattalion([building]);
	}
};
