 ...
 */
Trainer.prototype.Item = function(templateName, count, trainer, HCSpawnData, metadata)
{
	this.count = count;
	this.templateName = templateName;
	this.trainer = trainer;
	this.HCSpawnData = HCSpawnData; // HC-code, add specific HCSpawnData
...
{
	// HC-code, add IsSpawning check + only run spawn when not spawning and when count was set to 0 in FinishSpawnOperations
	let cmpTrainer = Engine.QueryInterface(this.trainer, IID_Trainer);
	if(cmpTrainer && !cmpTrainer.IsSpawning) {
		if (!this.count)
			this.finished = true;
		else this.Spawn();
 	}
...
{
	// HC-code, Removed a bunch of elements that are handled by Trainer_Hyrule
	// So instead, we run our custom initiation
	this.InitiateHyruleSpawn();
};
...
Trainer.prototype.Item.prototype.GetBasicInfo = function()
{
	return {
		...
		"paused": this.paused,
		"metadata": this.metadata,
		"battalionSlots": this.population, // HC-code, battalionslots for AI GetQueue
		"resources": this.resources // HC-code, resources for AI GetQueue
	};
};
 ...
 */
// HC-Code: We overwrite the function whole function to hide heroes/units which are tied to hero selection and shall not be displayed
...
//	return Array.from(this.entitiesMap.values());
	...
	let templateData = cmpTemplateManager.GetTemplate(template);
    if (!templateData) {
		continue;	}	
	let templateIdentity = templateData.Identity;
	    if (!templateIdentity) {
		continue;	}
	let hideIfTechnologyRequirementNotMet = templateIdentity.HideIfTechnologyRequirementIsNotMet;
...
{
	//HC-code, add spawn data to item
	const HCSpawnData = { "destinationOffsets": this.destinationOffsets, "spawnOffset": this.template.SpawnOffset, "lineupOffset": this.template.LineupOffset };
	const item = new this.Item(templateName, count, this.entity, HCSpawnData, metadata);
	//HC-end
