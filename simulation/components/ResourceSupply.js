...
ResourceSupply.prototype.Init = function()
{
	...
	this.activeGatherers = new Set(); // Battalion IDs
...
{
	// HC-Code: We use Set instead of Array
...
{
	// HC-Code: We use Set instead of Array
	return this.activeGatherers.size;
};
...
{
    // HC-Code
    // The resource needs to be available and this singular gatherer cant already be gathering from here to make it valid for a gathering order
    // Since HC is battalion centric and UnitAI is still entity centric, we have to check for a specific gatherer here rather than one in the entire battalion
    return this.IsAvailable(gathererID) == true && this.gatherers.has(gathererID) == false;
};
...
{
	// HC-Code
    if (this.IsGatheringUs(gathererID) && this.amount) // if this battalion is already gathering from this resource, return true by default
        return true;

	return this.gatherers.size < this.GetMaxGatherers(); // HC-Code if not, check if there is still another spot available
};
...
ResourceSupply.prototype.Change = function(change)
{
	...
	if (this.amount == 0){
		//HC-Code: Needed especially for Kokirir Berry Bushes to respawn
		let cmpHealth = Engine.QueryInterface(this.entity, IID_Health);
		if (cmpHealth){
			cmpHealth.CheckIntervalSpawnInvolvement();
		}
		// HC-End
...
ResourceSupply.prototype.AddGatherer = function(gathererID)
{
	...
	// HC-Code: We want to count battalions instead of individual units
	let cmpBattalion = Engine.QueryInterface(gathererID, IID_Battalion);
	let battalionID = cmpBattalion.ownBattalionID;
	
	if ( !this.gatherers.has(battalionID) )
	{
		this.gatherers.set(battalionID, new Set());
		this.gatherers.get(battalionID).add(gathererID);
		...
		if ( !this.gatherers.get(battalionID).has(gathererID)){
			this.gatherers.get(battalionID).add(gathererID);
		}
	}
	// HC-End
...
ResourceSupply.prototype.AddActiveGatherer = function(entity)
{
	...
	if ( !this.activeGatherers.has(battalionID) )
	{
		this.activeGatherers.add(battalionID);
		this.CheckTimers();
	}
	//HC-End
	return true;
...
ResourceSupply.prototype.RemoveGatherer = function(gathererID)
{
	...
	// If battalion is gathering
	if (this.gatherers.has(battalionID)){
		this.gatherers.get(battalionID).delete(gathererID);

		if (this.gatherers.get(battalionID).size == 0){
			this.gatherers.delete(battalionID);
			
			if(this.activeGatherers.has(battalionID)){
				this.activeGatherers.delete(battalionID, 1);
				this.CheckTimers();
			}
