...
function updateSelectionDetails()
{
	...
	let selectedEntities = new Array();	// HC-Code: Needed for battalion code to only show multi selection when multiple battalions are selected
	...
	for (let sel of g_Selection.toList())
	{
		...
		selectedEntities.push(sel); // HC-Code
	}
	...
	if (entStates.length == 1)
		...
		// HC-Code (else path; if is default 0ad): Only shows the multi selection, when the selected entities belong to multiple battalions
		if (EntitiesBelongToTheSameBattalion(selectedEntities)){
			displaySingle(entStates[0]);
		} else{
