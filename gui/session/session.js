...
function GetTemplateData(templateName, player)
{
	if (!(templateName in g_TemplateData))
	{
		...
		if (!template)
		{
			warn("GetTemplateData failed for template '" + templateName + "'.");
			g_TemplateData[templateName] = undefined;
			return undefined;
		}
...
function init(initData, hotloadData)
{
	...
	//HC-Code: Open the Hero Selection menu
    initCivChoicesDialog();

    //HC-Code: add arrays for every player on the field so the data can be added to the dialog queue
    for (let i = 0; i < GetSimState().players.length; i++)
        g_AIDialogQueue[i] = [];
...
function onSimulationUpdate()
{
	...
	//HC-Code Add or remove Battalions from the battalion UI if needed
	if (Engine.GuiInterfaceCall("IsBattalionListDirty")){
		...
		let battalionOrders = Engine.GuiInterfaceCall("GetBattalionOrders");
		
		UpdateBattalionList(battalionsToAdd, battalionsToUpdate, battalionsToDelete, battalionOrders);
	...
	//HC-Code:
	AddAverageGatherRateToResourceCounter();
}
