...
g_SelectionPanels.Construction = {
	...
	"setupButton": function(data)
	{
		...
		// HC-Code Adding battalion slots here allows buildings to take up battalion slots and disable the button in the construction panel if there are not enough slots left
		let notEnoughBattalionSlots = false;

		let neededResources;
		if (template.cost){ // HC-Code: Added "{ ... }"
			neededResources = Engine.GuiInterfaceCall("GetNeededResources", {
				"cost": multiplyEntityCosts(template, 1),
				"player": data.player
			});
			// HC-Code 
			let freeBattalionSlots = Engine.GuiInterfaceCall("GetFreeBattalionSlots");
			let neededBattalionSlots = template.cost.battalionSlots;

			// HC-Code We must check for (neededBattalionSlots > 0) to allow structures to be placed even if we have too many battalions on the field (11/10)
			if ((neededBattalionSlots > 0) && (freeBattalionSlots < neededBattalionSlots)){
				notEnoughBattalionSlots = true;
			}
		}
		...
		let modifier = "";
		// HC-Code: Added check for notEnoughBattalionSlots
		if (!technologyEnabled || limits.canBeAddedCount == 0 || notEnoughBattalionSlots)
		{
