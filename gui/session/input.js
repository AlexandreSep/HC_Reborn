...
function updateBuildingPlacementPreview()
{
	...
	if (placementSupport.mode === "building")
	{
		if (placementSupport.template && placementSupport.position)
		{
			...
			// HC-Code - Prevent structures to be placed when there are no battalion slots left
			let template = GetTemplateData(placementSupport.template);
...
function performGroup(action, groupId)
{
	...
	case "breakUp":
		...
		// HC-Code We want to group the battalions in the battalion UI
		UngroupInBattalionUI (groupId);
		
		if (action == "save"){
			g_Groups.addEntities(groupId, g_Selection.toList());
			GroupInBattalionUI (groupId, g_Selection.toList()); //HC-Code
		}
